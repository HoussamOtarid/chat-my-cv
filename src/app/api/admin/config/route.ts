...
export async function PUT(request: NextRequest) {
    ...
    try {
        ...
        const { data: existing } = await supabase.from('configuration').select('id').single();
        ...
        let result;
        if (existing) {
            result = await supabase.from('configuration').update(configData).eq('id', existing.id).select().single();
        } else {
            result = await supabase.from('configuration').insert(configData).select().single();
        }
